...
function EmployeeDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate leave duration does not exceed remaining leave balance
    const daysRequested =
      Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24)) + 1;

    if (daysRequested > remainingLeaves[leaveType]) {
      alert(
        `You only have ${remainingLeaves[leaveType]} ${leaveType} leave days remaining.`
      );
      return;
    }
              ...
              <tbody>
                ...
                {leaveRequests.map((leave) => (
                  ...
                  >
                    ...
                    <td style={{ padding: 16 }}>{formatDate(leave.start_date)}</td>
                    <td style={{ padding: 16 }}>{formatDate(leave.end_date)}</td>
